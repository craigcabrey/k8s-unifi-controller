...
impl PortForwardRule {
    ...
    pub async fn create(&self, client: &UnifiClient) -> Result<PortForwardRule> {
        ...
        let response = client
            ...
            .await?
            .error_for_status()?;
        let api_response = response.json::<UnifiApiResponse<PortForwardRule>>().await?;
        if !api_response.meta.is_ok() {
            return Err(anyhow!("Unifi API error: {:?}", api_response.meta.rc));
        }
        info!("Created port forward rule '{:?}'", api_response.data);
        Ok(api_response.data)
    }
    ...
    pub async fn update(&self, client: &UnifiClient) -> Result<PortForwardRule> {
        ...
        info!("Updated port forward rule '{:?}'", api_response.data);
        Ok(api_response.data)
    }
...
impl FirewallGroup {
    ...
    pub async fn create(&self, client: &UnifiClient) -> Result<FirewallGroup> {
        ...
        let response = client
            ...
            .await?
            .error_for_status()?;
        let api_response = response.json::<UnifiApiResponse<FirewallGroup>>().await?;
        if !api_response.meta.is_ok() {
            return Err(anyhow!("Unifi API error: {:?}", api_response.meta.rc));
        }
        info!("Created firewall group '{:?}'", api_response.data);
        Ok(api_response.data)
    }
    ...
    pub async fn update(&self, client: &UnifiClient) -> Result<FirewallGroup> {
        ...
        info!("Updated firewall group '{:?}'", api_response.data);
        Ok(api_response.data)
    }
